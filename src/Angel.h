...
//uint InitShader( const char* vertexShaderFile,
...
int
_gluUnProject(double winx, double winy, double winz,
             const double modelMatrix[16],
             const double projMatrix[16],
             const int viewport[4],
...
static void check_shader_compilation(std::string shader_file, uint shader){
  int  compiled;
  glGetShaderiv( shader, COMPILE_STATUS, &compiled );
  if ( !compiled ) {
    std::cerr << shader_file.c_str() << " failed to compile:" << std::endl;
    int  logSize;
    glGetShaderiv( shader, INFO_LOG_LENGTH, &logSize );
...
static void check_program_link(uint program){
  
  int  linked;
  glGetProgramiv( program, LINK_STATUS, &linked );
  if ( !linked ) {
    std::cerr << "Shader program failed to link" << std::endl;
    int  logSize;
    glGetProgramiv( program, INFO_LOG_LENGTH, &logSize);
    char* logMsg = new char[logSize];
